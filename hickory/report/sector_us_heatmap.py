            ...
            if (i*4 + j < len(industries)):
                ...
                if (i*4 + j < len(industries)):
                    ...
                    for stock in stocks:
                        ...
                        #print(lcp)
                        if (lvr):
                            if (datetime.datetime.now().hour >= 13 and float(lcp.replace("%","")) < 0 and float(lvr) < 0.5):
                                style_bg = "background-color: #FFFFB2;"
                            elif (float(lcp.replace("%","")) < -2 and float(lvr) > 3):
                                style_bg = "background-color: #F1B3B3;"
                            elif (float(lcp.replace("%","")) < -2 and float(lvr) > 1):
                        ...
                        if (not stock["_52WEEK_HIGH"] == None and (stock["LAST_CLOSE"] > stock["_52WEEK_HIGH"])):
