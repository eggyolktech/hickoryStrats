...
def generate_SIG_MT(num_workers=1):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        future_to_manage = {executor.submit(manageStockSignals, row["code"], 4): row for row in rows[:LIMIT]}
        ...
        try:
            ...
            for d in data_list:
                ...
                if (d[1] == True):
                    p1 = p1 + "/qd" + rcode + " - " + stock_db.get_stock_name(rcode) + " (" + stock_db.get_stock_industry(rcode) + ")" + '\n'
                if (d[2] == True):
            ...
            if (p1):
                bot_sender.broadcast(str(t1 + p1))
            if (p2):
                bot_sender.broadcast(str(t2 + p2))
            if (p3):
                bot_sender.broadcast(str(t3 + p3))
